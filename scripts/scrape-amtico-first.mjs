// scrape-amtico-first.js
// Run (prints JSON to stdout):
//   HEADFUL=true node scripts/scrape-amtico-first.js > amtico-first-variants.json
//
// Output shape:
...
const URL = "https://www.amtico.com/commercial/products/?collection=Amtico+First";
const PRODUCT_FOLDER = "first";
const EXPECTED_COUNT = 26;
const OUTFILE = "amtico-first-variants.json";

// Minimal but distinct (neutrals/woods/greys + a few muted accents)
...
async function chooseEnglishIfPresent(page) {
    try {
        const dialog = page
            .locator('[role="dialog"], [aria-modal="true"], .modal, .Modal, .MuiDialog-root')
            .first();
...
function pickBestName({ heading, imgAlt, cardText, code }) {
    ...
    if (!name || name.length < 3) name = `Amtico First ${code}`;
...
async function main() {
    ...
    // Load-more can appear more than once
    await clickLoadMoreIfPresent(page);
